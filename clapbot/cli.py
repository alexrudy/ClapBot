...
@click.option(
    "--limit",
    type=int,
    default=app.config['CRAIGSLIST_MAX_SCRAPE'],
    help="Limit the number of scraped results.")
...
def scrape_command(limit, to_json=False):
    ...
    if to_json:
        path = os.path.join(app.instance_path,
                            app.config['CRAIGSLIST_CACHE_PATH'])
...
@click.option(
    "--path",
    default=app.config['CRAIGSLIST_CACHE_PATH'],
    type=click.Path(),
    help="Path to import entries to db.")
def ingest_command(path):
    ...
    stream = io.TextIOWrapper(
        pkg_resources.resource_stream(__name__, "data/boxes.csv"))
